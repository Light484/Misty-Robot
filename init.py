...
class Robot:
    ...
    def moveArm(self, arm, position, velocity):
       assert arm in ["left", "right"], " moveArm: Arm should be either 'left' or 'right'"
       assert position in range(-90, 91), " moveArm: Position should be in range 0 to 360"
       assert velocity in range(0, 100), " moveArm: Velocity should be in range 0 to 100"
       requests.post('http://'+self.ip+'/api/arms',json={"Arm": arm, "Position": position, "Velocity": velocity})

    def speak(self, text, pitch=0, speech_rate=0, voice=None, flush=False, utterance_id=None, language=None):
        """
        Makes Misty speak the provided text using her onboard text-to-speech engine.
        
        Args:
            text (str): The text for Misty to speak.
            pitch (int): The pitch adjustment (-10 to 10).
            speech_rate (int): The rate of speech (-10 to 10).
            voice (str): The voice to use (if multiple voices are supported).
            flush (bool): If True, stops any current speech and begins speaking immediately.
            utterance_id (str): An identifier for the utterance to track events.
            language (str): The language to use (e.g., "en-US").
        """
        # Ensure text is a string
        assert isinstance(text, str), "speak: The 'text' parameter must be a string."
        ...
        response = requests.post(f'http://{self.ip}/api/tts/speak', json=payload, timeout=10)
        
        if response.status_code == 200:
            print("Misty is speaking.")
        else:
            print(f"Failed to make Misty speak. HTTP Status Code: {response.status_code} | Response: {response.text}")
    ...
    #   "RightArmVelocity": 5
    # }
       assert LeftArmPosition in range(-90, 90)
       assert RightArmPosition in range(-90, 90)
       assert LeftArmVelocity in range(-20, 21)
       assert RightArmVelocity in range(-20, 21)
       requests.post('http://'+self.ip+'/api/arms/set',json={"LeftArmPosition": LeftArmPosition, "RightArmPosition": RightArmPosition, "LeftArmVelocity": LeftArmVelocity, "RightArmVelocity": RightArmVelocity})
